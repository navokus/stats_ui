...
class PayFrequency extends MY_Controller {
	...
	public function index()
	{
		...
		if ($this->input->post('options')) {
			$_SESSION['arppu']['post'] = $_POST;
			redirect('PayFrequency');
		}

		if ($_SESSION['arppu']['post']) {
			$return['post'] = $_SESSION['arppu']['post'];
		} else {
		...
		switch ($return['post']['options']) {
			case '5': // week
				...
				$return['payFrequency'] = $this->retention->getDataPayFrequency($gameCode, $return['post']['options'], $return['post']['week']);
				$return['payMethod']    = $this->retention->getDataPayMethod($gameCode, $return['post']['options'], $return['post']['week']);
			...
			case '6': // month
				...
				// get data pay frequency
				$return['payFrequency'] = $this->retention->getDataPayFrequency($gameCode, $return['post']['options'], $return['post']['month']);
				$return['payMethod']    = $this->retention->getDataPayMethod($gameCode, $return['post']['options'], $return['post']['month']);
			...
			default:
				...
				// get data pay frequency
				$return['payFrequency'] = $this->retention->getDataPayFrequency($gameCode, $return['post']['options'], $formatDay);
				$return['payMethod']    = $this->retention->getDataPayMethod($gameCode, $return['post']['options'], $formatDay);

				$return['time'] = 'Ngày';
				$return['timing'] = 'daily';
				break;
		}
		$return['CalculateDate'] = $formatDay;
		$return['gameCode'] = strtoupper($gameCode);
		$this->_template['content'] = $this->load->view('pay_frequency/index', $return, TRUE);
