...
class Compare extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('paying_detail_grade_model', 'paying_detail_grade');
	...
	public function index($gameCode, $times, $preDate, $curDate)
	{
		...
		$data['aGrade'] = $aGrade['pay'][$times];
		$data['preDate'] = $preDate;
		$data['curDate'] = $curDate;
		$data['aPayCompare'] = $this->paying->compare($gameCode, $times, $_preDate, $_curDate);		

		$data['fileExportCurDate'] = 'revenue_'. $gameCode . '_' . $curDate . '_' . $times;	
		$data['fileExportPreDate'] = 'revenue_'. $gameCode . '_' . $preDate . '_' . $times;
		
		if($TimeConsecutive == 1) {
			$data['content_paying_detail'] = $this->paying_detail($gameCode, $times, $_preDate, $_curDate, $data['color'], $data['times'], $data['curDate'], $data['key']);
		}		

		if ($_SESSION['paycompare'][$data['key']]) {
			unset($_SESSION['paycompare'][$data['key']]);
		}

		$_SESSION['paycompare'][$data['key']]['content'] = $this->load->view('compare', $data, TRUE);
		$_SESSION['paycompare'][$data['key']]['header'] = $data['gameCode'] . '_' . $data['times'] . '_' . $data['curDate'] . '_' . $data['preDate'];

	}

	public function delCacheCompare($key)
	{
		if (isset($_SESSION['paycompare'][$key])) {
			unset($_SESSION['paycompare'][$key]);
		}
		redirect(site_url('Paying/compare/'));
	}

	public function paying_detail($gameCode, $times, $preDate, $curDate, $color, $timeName, $dateName, $key)
	{
		$data['color'] = $color;
		$data['timeName'] = $timeName;
		$data['curDate'] = $dateName;
		$data['key'] = $key;
		$data['aPayDetailGrade'] = $this->paying_detail_grade->getPaymentDetailGrade($gameCode, $times, $curDate);		
		$data['aPayDetailCompare'] = $this->paying_detail->compare($gameCode, $times, $preDate, $curDate);
	...
	{
		list($reportType, $gameCode, $curDate, $times, $grade) = explode('_', $fileName);
