...
<div class="box box-solid bg-green-gradient">
	<div class="box-header">
		<h3 class="box-title">SO SÁNH CHI TRẢ</h3>

	</div><!-- /.box-header -->
	<form name="paying" action="<?php echo site_url('Paying/compare/') ?>" method="post">
			...
			<ul class="nav nav-tabs">
				<?php  
					...
					foreach ($aPayCompare as $key => $value) {
						...
						echo '
						<li class="'.$active.'">
							<a href="'. site_url('Paying/compare/' . $key) . '" >['.strtoupper($gameCode) .'] '. $times . ' ' . $preDate. ' - ' . $curDate .'</a>
							<button onclick="window.location.href=\''.site_url('paying_module/Compare/delCacheCompare/' . $key).'\'" class="del-pay btn btn-box-tool" data-toggle="tooltip" title="Remove" style="position:absolute; top:-5px; right:0px"><i class="fa fa-times"></i></button>
						</li>';
