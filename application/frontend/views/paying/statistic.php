...
<div class="box box-solid bg-green-gradient">
	<div class="box-header">
		<h3 class="box-title">THỐNG KÊ CHI TRẢ</h3>

	</div><!-- /.box-header -->
	<form name="paying" action="<?php echo site_url('Paying/statistic/') ?>" method="post">
	...
	<!-- Custom Tabs -->
		<div class="nav-tabs-custom">
			<ul class="nav nav-tabs">
				<?php  
					$aPay = array_reverse($_SESSION['payStatistic']);
					$i = 1;
					foreach ($aPay as $key => $value) {
						...
						echo '
						<li class="'.$active.'">
							<a href="'. site_url('Paying/statistic/' . $key) . '" >['.strtoupper($gameCode) .'] '. $times . ' ' . $curDate .'</a>
							<button onclick="window.location.href=\''.site_url('paying_module/Statistic/delCacheStatistic/' . $key).'\'" class="del-pay btn btn-box-tool" data-toggle="tooltip" title="Remove" style="position:absolute; top:-5px; right:0px"><i class="fa fa-times"></i></button>
						</li>';
